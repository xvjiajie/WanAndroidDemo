...
//                Logc.i(entry.getKey() + " -> " + value);
            ...
            TreeMap<String, String> rootMap = new TreeMap<>();
            RequestBody body = request.body();
            Buffer buffer = new Buffer();
            try {
                body.writeTo(buffer);
                String oldJsonParams = buffer.readUtf8();
                rootMap = new Gson().fromJson(oldJsonParams, TreeMap.class); // 原始参数
                TreeMap<String, String> newParams = dynamic(rootMap);
                String newJsonParams = new Gson().toJson(newParams);
                request = request.newBuilder().post(Utils.createJson(newJsonParams)).build();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }*/
